...
export class ArloCameraAccessory extends ArloPlatformAccessory {
  ...
  constructor(
    ...
    this.services['cameraControl'].getCharacteristic(this.platform.Characteristic.On)
      .on('set', this.setOn.bind(this)); // SET - bind to the `setOn` method below

    // register handlers for the Image Mirroring Characteristic
    this.services['cameraControl'].getCharacteristic(this.platform.Characteristic.ImageMirroring)
    ...
    this.services['cameraControl'].getCharacteristic(this.platform.Characteristic.ImageRotation)
    ...
    this.device.on(Arlo.BATTERY, (batteryLevel) => {
      ...
      this.services['batteryService'].updateCharacteristic(this.platform.Characteristic.BatteryLevel, batteryLevel);

      this.platform.log.debug('Pushed updated current Battery Level state to HomeKit:', batteryLevel);
    });
    ...
    this.device.on(Arlo.CHARGING, (value) => {
      ...
      if (value) {
        chargingState = value !== 'Off'
          ? this.platform.Characteristic.ChargingState.CHARGING
          : this.platform.Characteristic.ChargingState.NOT_CHARGING;
      }

      // push the new value to HomeKit
      this.services['batteryService'].updateCharacteristic(this.platform.Characteristic.ChargingState, chargingState);

      this.platform.log.debug('Pushed updated current Charging state to HomeKit:', chargingState);
    });
    ...
    this.device.on(Arlo.MOTION, (motionDetected) => {
      ...
      // push the new value to HomeKit
      this.services['motionSensor'].updateCharacteristic(this.platform.Characteristic.MotionDetected, motionDetected);

      this.platform.log.debug('Pushed updated current Motion Detected state to HomeKit:', motionDetected);
    });
    ...
    this.device.on(Arlo.UPDATE, (info) => {
      ...
      if (info.hwVersion) {
        this.accessory.getService(this.platform.Service.AccessoryInformation)!
          .setCharacteristic(this.platform.Characteristic.HardwareRevision, info.hwVersion);
      }

      if (info.swVersion) {
        this.accessory.getService(this.platform.Service.AccessoryInformation)!
          .setCharacteristic(this.platform.Characteristic.SoftwareRevision, info.swVersion);
      }

      if (info.privacyActive) {
        // push the new value to HomeKit
        this.services['cameraControl'].updateCharacteristic(this.platform.Characteristic.On, info.privacyActive === false);
      }

      if (info.mirror) {
        // push the new value to HomeKit
        this.services['cameraControl'].updateCharacteristic(this.platform.Characteristic.ImageMirroring, info.mirror);
      }

      if (info.flip) {
        // push the new value to HomeKit
        this.services['cameraControl'].updateCharacteristic(this.platform.Characteristic.ImageRotation, info.flip === true ? 180 : 0);
      }

      if (info.nightVisionMode) {
        // push the new value to HomeKit
        this.services['cameraControl'].updateCharacteristic(this.platform.Characteristic.NightVision, info.nightVisionMode === 1);
      }

      if (info.connectionState) {
        const online = info.connectionState === 'available';
        this.platform.log.info(`${online ? 'Online' : 'Offline'}: Camera ${this.accessory.displayName} [${this.device.id}]`);
      }
    });
    ...
    this.accessory.configureCameraSource(new ArloCameraSource(platform, accessory));
  }
   ...
   */
  setImageMirroring(value: CharacteristicValue, callback: CharacteristicSetCallback) {
   ...
   */
  setImageRotation(value: CharacteristicValue, callback: CharacteristicSetCallback) {
